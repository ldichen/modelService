...
class OGMSTaskAccess(Service):
    ...
    def subscribeTask(self, task: OGMSTask) -> ResultUtils:
        ...
        if res is None:
            ...
            if res["code"] == 0:
                ...
                for fileId in res["output"]:
                    url = f"http://{self.dataServer}:{self.dataPort}/data/" + fileId
    ...
    def downloadAllData(self) -> list:
        ...
        for output in self.outputs:
            ...
            content, content_disposition = HttpHelper.Request_get_url_sync(output)
            ...
            if content:
                ...
                file_path = "./data/" + self.methodName + "_" + s_id + "/" + filename
                dir_path = os.path.dirname(file_path)
                if not os.path.exists(dir_path):
                    os.makedirs(dir_path)
                with open(file_path, "wb") as f:
                    f.write(content)
                outputInfo.append({"filename": filename, "url": file_path})
